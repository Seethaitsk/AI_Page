...
// ============================================================
document.addEventListener("DOMContentLoaded", () => {
    const carouselContainer = document.getElementById('industryCarousel');
    if (!carouselContainer) return;

    const cards = Array.from(carouselContainer.querySelectorAll('.card-container'));
    ...
    function updateCarousel() {
        ...
        cards.forEach((card, i) => {
            // Shortest path for circular wrapping
            let diff = active - i;
            ...
            card.style.opacity = Math.abs(diff) >= MAX_VISIBILITY ? '0' : '1';
